...
import { HypernetContext, InitializedHypernetContext } from "./HypernetContext";
import { account, activeStateChannel, publicIdentifier } from "@mock/mocks";
import { okAsync, ResultAsync } from "neverthrow";
import { Subject } from "rxjs";

export class ContextProviderMock {
	public context: any;
	public initializedContext: any;
	...
	public onGatewayConnectorActivated: Subject<any>;
	public onGatewayConnectorActivatedActivations: any[] = [];
	...
	constructor(
		...
		if (context != null) {
			...
			this.context = new HypernetContext(
				...
				chainConfig.get(ChainId(1337)) as GovernanceChainInformation,
	...
	public assertEventCounts(expectedCounts: IExpectedEventCounts): void {
		...
		expect(this.onChainChangedActivations.length).toBe(
			counts.onChainChanged,
		);
